...
vec4 createContentNormal_h4h2h_f4f2f(vec2 pos, float maskAlpha) {
    if (maskAlpha < 0.01) {
...
vec4 ContentShinning_h4h2h4hh3h3f33h_f4f2f4ff3f3f33f(vec2 uv, vec4 specularColor, float shinning, vec3 lightPos, vec3 viewPos, mat3 rotM, float maskAlpha) {
    vec3 fragPos = vec3(uv, 0.0);
    vec4 normal = createContentNormal_h4h2h_f4f2f(uv, maskAlpha);
    if (normal.w < 0.0) {
        return vec4(0.0);
    }
...
void main() {
    ...
    vec4 inputImage = texture(image, (fragCoord) / iResolution);
    ...
    shinningColor = ContentShinning_h4h2h4hh3h3f33h_f4f2f4ff3f3f33f(uv, specularColor, shinning, lightPos, viewPos, rotM, inputImage.w);
