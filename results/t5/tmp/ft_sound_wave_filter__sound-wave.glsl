...
vec3 colorWheel_h3h2h_f3f2f(vec2 uv, float animationTime) {
...
vec4 soundWaveDistortionEffects_h4h2h2h_f4f2f2f(vec2 screenUVs, vec2 centeredUVs, float animationTime) {
...
void main() {
    ...
    uv.y = 1.0 - uv.y;
    float screenRatio = (0.25 * iResolution.x) / iResolution.y;
    vec2 centeredUVs = (uv + uv) - vec2(1.0, 0.0);
    centeredUVs.y *= 0.25;
    centeredUVs.x *= screenRatio;
    float additionalColorStrength = 0.0;
    if (shockWaveTotalAlpha > 0.0) {
        vec4 soundWaveDistortionA = soundWaveDistortionEffects_h4h2h2h_f4f2f2f(uv, centeredUVs, shockWaveProgressA);
        vec4 soundWaveDistortionB = soundWaveDistortionEffects_h4h2h2h_f4f2f2f(soundWaveDistortionA.xy, centeredUVs, shockWaveProgressB);
    ...
    vec3 finalColor = texture(image, vec2(uv.x, 1.0 - uv.y)).xyz;
    vec3 centerColor = additionalColorStrength > 0.0 ? colorWheel_h3h2h_f3f2f(centeredUVs, colorProgress) : vec3(0.0);
