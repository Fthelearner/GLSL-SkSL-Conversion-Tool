...
void main() {
    ...
    vec2 grad = vec2(texture(imageInput, (centerFragCoord + vec2(0.5, 0.0)) / vec2(textureSize(imageInput, 0))).w - texture(imageInput, (centerFragCoord - vec2(0.5, 0.0)) / vec2(textureSize(imageInput, 0))).w, texture(imageInput, (centerFragCoord + vec2(0.0, 0.5)) / vec2(textureSize(imageInput, 0))).w - texture(imageInput, (centerFragCoord - vec2(0.0, 0.5)) / vec2(textureSize(imageInput, 0))).w);
    vec2 normGrad = normalize(grad);
    vec2 edgeCoords = vec2(0.5);
    float borderCoeff = (imageSample - 0.5) * 1.41421354;
