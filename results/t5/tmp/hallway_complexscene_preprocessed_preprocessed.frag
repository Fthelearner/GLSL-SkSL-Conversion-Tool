...
void mainImage0(out vec4 o, vec2 u) {
    float i = 0.0, a = 0.0, d = 0.0, s = 0.0, t=iTime;
    vec3  p = iResolution;
    u = (u-p.xy/2.)/p.y;
    for(o*=i; i++<1e2;
        d += s = .01+abs(s) * .6,
...
void mainImage1(out vec4 o, vec2 u) {
    float d = 0.0, a = 0.0, e = 0.0, i = 0.0, s = 0.0, t = iTime*.5;
    vec3 ep = vec3(0.0, 0.0, 0.0), p = iResolution;
    u = (u+u-p.xy)/p.y;    
...
void mainImage2(out vec4 o, vec2 u) {
    float i = 0.0, d = 0.0, s = 0.0, n = 0.0, t = iTime, a = 0.0;
    vec3 p = iResolution;   
    u = (u-p.xy/2.)/p.y;  
...
{
    float s=.002, d=0., i = 0.0, l = 0.0, w = 0.0, t=iTime*4., f=(sin(iTime*1.5)*35.+60.);
    vec3  r = iResolution,
        p = P(T),ro = p,
...
void mainImage4(out vec4 o, in vec2 u) {
   float l = 0.0, i = 0.0, d = 0.0, s=.002, T = iTime*6.;
    vec3  q = iResolution,
          p = P(T),ro=p,
          Z = normalize( P(T+4.) - p),
          X = normalize(vec3(Z.z,0,-Z)),
...
void mainImage5(out vec4 o, in vec2 u) {
    float l = 0.0, i = 0.0, d = 0.0, s=.002, T = iTime*9.;
    vec3  q = iResolution,
          p = P(T),ro=p,
          Z = normalize( P(T+4.) - p),
          X = normalize(vec3(Z.z,0,-Z)),
...
void mainImage6(out vec4 o, vec2 u) {
    float l = 0.0, s = 0.0, d = 0.0, i = 0.0, n = 0.0;
    vec3 p = iResolution;
    u = (u-p.xy/2.)/p.y;
...
void mainImage7(out vec4 o, vec2 u) {
    float i = 0.0, d = 0.0, s = 0.0, n = 0.0, t = iTime;
    vec3 p = iResolution;
    u = (u-p.xy/2.)/p.y;
...
void mainImage9(out vec4 o, vec2 u) {
    float d = 0.0, i = 0.0, e = 0.0, s = 0.0, w = 0.0, l = 0.0, T = iTime;
    vec3  p = iResolution;
    u = ( u - p.xy/2. ) / p.y
       + vec2(sin(T*.2)*.3,
              sin(T*.5)*.1);
...
void mainImage10(out vec4 o, vec2 u) {
    ...
    vec3 p = vec3(0.0, 0.0, 0.0), r = iResolution;
    for(o*=i;
        i++<64.;
