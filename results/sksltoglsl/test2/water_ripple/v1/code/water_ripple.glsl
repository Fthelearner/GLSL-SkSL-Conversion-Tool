...
float calc_wave_ff(float distance_to_wave) {
...
void main() {
    ...
    float _0_distance_to_wave = propagated_distance - WAVE_PROPAGATION_RATIO * time_value;
    const float _1_delta = 0.001;
    float _2_d1 = _0_distance_to_wave - _1_delta;
    float _3_d2 = _0_distance_to_wave + _1_delta;
    float intensity = ((((calc_wave_ff(_3_d2) - calc_wave_ff(_2_d1)) * 499.999969) * amplitude_decay) * amplitude_suppress) * AMPLITUDE_SUPPRESS;
    ...
    vec2 distorted_coord = (uv - DISTORT_INTENSITY * normal.xy) * iResolution;
