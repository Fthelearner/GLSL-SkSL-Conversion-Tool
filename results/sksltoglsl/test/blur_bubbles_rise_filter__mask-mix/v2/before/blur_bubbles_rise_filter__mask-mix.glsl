...
void main() {
    ...
    vec4 maskColor = texture(blur_mask, (maskUV * iResolution) / vec2(textureSize(blur_mask, 0)));
    float maskValue = maskColor.x;
    float blend = clamp(maskValue * mixStrength, 0.0, 1.0);
    vec4 originColor = texture(original_tex, (fragCoord) / vec2(textureSize(original_tex, 0)));
    vec4 blurredColor = texture(blur_tex, (fragCoord) / vec2(textureSize(blur_tex, 0)));
