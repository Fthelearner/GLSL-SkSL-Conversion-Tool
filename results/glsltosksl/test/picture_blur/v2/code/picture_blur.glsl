    ...
    if (transition > 0.0) {
        ...
        for (float x = -4.0;x <= 4.0; x++) {
            for (float y = -4.0;y <= 4.0; y++) {
                ...
                fragColor += texture(iChannel0, (iResolution.xy * (offset + lens)) / vec2(textureSize(iChannel0, 0)));
                total += 1.0;
            }
        }
        fragColor /= total;
        float gradient = clamp((clamp(m2.y, 0.0, 0.2) + 0.1) * 0.5, 0.0, 1.0) + clamp((clamp(-m2.y, -1000.0, 0.2) * rb3 + 0.1) * 0.5, 0.0, 1.0);
        vec4 lighting = clamp((fragColor + vec4(rb1) * gradient) + vec4(rb2) * 0.3, vec4(0.0), vec4(1.0));
        fragColor = mix(texture(iChannel0, (iResolution.xy * uv) / vec2(textureSize(iChannel0, 0))), lighting, vec4(transition));
    } else {
        fragColor = texture(iChannel0, (iResolution.xy * uv) / vec2(textureSize(iChannel0, 0)));
    }
