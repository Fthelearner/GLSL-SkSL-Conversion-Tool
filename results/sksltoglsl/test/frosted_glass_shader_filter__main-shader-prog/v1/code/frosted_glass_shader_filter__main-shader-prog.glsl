...
uniform float hlB;
uniform float hlS;
vec3 Sat_f3f3fffffff(vec3 src255, float n, float p1r, float p2r, float p1g, float p2g, float p1b, float p2b) {
    float r = src255.x;
    float g = src255.y;
    float b = src255.z;
...
vec3 ApplyKBS_f3f3fffffffff(vec3 c01, float K, float B, float S, float p1r, float p2r, float p1g, float p2g, float p1b, float p2b) {
    vec3 x = c01 * 255.0;
    x = x * K + vec3(B);
    x = Sat_f3f3fffffff(x, S, p1r, p2r, p1g, p2g, p1b, p2b);
    return clamp(x * 0.003921569, vec3(0.0), vec3(1.0));
}
vec3 BlurVibrancy_f3f3(vec3 c01) {
    vec3 x = c01 * 255.0;
...
void main() {
    ...
    float border = smoothstep(-0.5, max(1.0, (borderWidth * ANTI_ALIASING) * 0.5), -sd * ANTI_ALIASING) - smoothstep(min((-borderWidth * ANTI_ALIASING) * 0.5, -1.0), 0.5, (-sd - borderWidth) * ANTI_ALIASING);
    float borderBlack = smoothstep(-0.5, max(1.0, (borderWidth * ANTI_ALIASING) * 0.5), -sdBlack * ANTI_ALIASING) - smoothstep(min((-borderWidth * ANTI_ALIASING) * 0.5, -1.0), 0.5, (-sdBlack - borderWidth) * ANTI_ALIASING);
    ...
    if (embossNeg > 0.0) {
        ...
        vec2 deltaInDS = (nOut * innerShadowRefractPx) / max(downSampleFactor, N_EPS);
        ...
        const vec3 _1_grayBase = vec3(0.2412016, 0.6922296, 0.0665688);
        float _2_lb = dot(blurredBgColor.xyz, _1_grayBase);
        float _3_le = dot(refractionNeg.xyz, _1_grayBase);
        refractionNeg.xyz = mix(blurredBgColor.xyz, refractionNeg.xyz, vec3(_3_le / max(_2_lb + _3_le, EPSILON)));
    ...
    if (embossPos > 0.0) {
        ...
        vec2 pixelDS = uvInTile * (tileSize - 1.0) + 0.5;
        vec2 nOut = sdfNormal.xy;
        vec2 deltaOutDS = (nOut * refractOutPx) / max(downSampleFactor, N_EPS);
        vec2 posCoord = pixelDS + deltaOutDS;
        vec4 refractionPos = texture(edgeBlurredImg, (posCoord) / iResolution) * bgFactor;
        const vec3 _4_grayBase = vec3(0.2412016, 0.6922296, 0.0665688);
        float _5_lb = dot(blurredBgColor.xyz, _4_grayBase);
        float _6_le = dot(refractionPos.xyz, _4_grayBase);
        refractionPos.xyz = mix(blurredBgColor.xyz, refractionPos.xyz, vec3(_6_le / max(_5_lb + _6_le, EPSILON)));
    ...
    float _7_a = max(highLightFeatherPx, N_EPS);
    ...
    vec3 hlBase = ApplyKBS_f3f3fffffffff(blurredBgColor.xyz, hlK, hlB, hlS, 1.0, 1.7, 1.5, 3.0, 2.0, 1.0);
