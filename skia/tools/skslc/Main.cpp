...
static const char* program_kind_name(SkSL::ProgramKind kind) {
    switch (kind) {
        case SkSL::ProgramKind::kFragment:                  return "Fragment";
        case SkSL::ProgramKind::kVertex:                    return "Vertex";
        case SkSL::ProgramKind::kCompute:                   return "Compute";
        case SkSL::ProgramKind::kGraphiteFragment:          return "GraphiteFragment";
        case SkSL::ProgramKind::kGraphiteVertex:            return "GraphiteVertex";
        case SkSL::ProgramKind::kRuntimeColorFilter:        return "RuntimeColorFilter";
        case SkSL::ProgramKind::kRuntimeShader:             return "RuntimeShader";
        case SkSL::ProgramKind::kRuntimeBlender:            return "RuntimeBlender";
        case SkSL::ProgramKind::kPrivateRuntimeColorFilter: return "PrivateRuntimeColorFilter";
        case SkSL::ProgramKind::kPrivateRuntimeShader:      return "PrivateRuntimeShader";
        case SkSL::ProgramKind::kPrivateRuntimeBlender:     return "PrivateRuntimeBlender";
        case SkSL::ProgramKind::kMeshVertex:                return "MeshVertex";
        case SkSL::ProgramKind::kMeshFragment:              return "MeshFragment";
    }
    SkUNREACHABLE;
}

static const char* program_element_kind_name(SkSL::ProgramElement::Kind kind) {
    switch (kind) {
        case SkSL::ProgramElement::Kind::kExtension:         return "Extension";
        case SkSL::ProgramElement::Kind::kFunction:          return "Function";
        case SkSL::ProgramElement::Kind::kFunctionPrototype: return "FunctionPrototype";
        case SkSL::ProgramElement::Kind::kGlobalVar:         return "GlobalVar";
        case SkSL::ProgramElement::Kind::kInterfaceBlock:    return "InterfaceBlock";
        case SkSL::ProgramElement::Kind::kModifiers:         return "Modifiers";
        case SkSL::ProgramElement::Kind::kStructDefinition:  return "StructDefinition";
    }
    SkUNREACHABLE;
}

static const char* statement_kind_name(SkSL::Statement::Kind kind) {
    switch (kind) {
        case SkSL::Statement::Kind::kBlock:          return "Block";
        case SkSL::Statement::Kind::kBreak:          return "Break";
        case SkSL::Statement::Kind::kContinue:       return "Continue";
        case SkSL::Statement::Kind::kDiscard:        return "Discard";
        case SkSL::Statement::Kind::kDo:             return "Do";
        case SkSL::Statement::Kind::kExpression:     return "Expression";
        case SkSL::Statement::Kind::kFor:            return "For";
        case SkSL::Statement::Kind::kIf:             return "If";
        case SkSL::Statement::Kind::kNop:            return "Nop";
        case SkSL::Statement::Kind::kReturn:         return "Return";
        case SkSL::Statement::Kind::kSwitch:         return "Switch";
        case SkSL::Statement::Kind::kSwitchCase:     return "SwitchCase";
        case SkSL::Statement::Kind::kVarDeclaration: return "VarDeclaration";
    }
    SkUNREACHABLE;
}

static const char* expression_kind_name(SkSL::Expression::Kind kind) {
    switch (kind) {
        case SkSL::Expression::Kind::kBinary:                    return "Binary";
        case SkSL::Expression::Kind::kChildCall:                 return "ChildCall";
        case SkSL::Expression::Kind::kConstructorArray:          return "ConstructorArray";
        case SkSL::Expression::Kind::kConstructorArrayCast:      return "ConstructorArrayCast";
        case SkSL::Expression::Kind::kConstructorCompound:       return "ConstructorCompound";
        case SkSL::Expression::Kind::kConstructorCompoundCast:   return "ConstructorCompoundCast";
        case SkSL::Expression::Kind::kConstructorDiagonalMatrix: return "ConstructorDiagonalMatrix";
        case SkSL::Expression::Kind::kConstructorMatrixResize:   return "ConstructorMatrixResize";
        case SkSL::Expression::Kind::kConstructorScalarCast:     return "ConstructorScalarCast";
        case SkSL::Expression::Kind::kConstructorSplat:          return "ConstructorSplat";
        case SkSL::Expression::Kind::kConstructorStruct:         return "ConstructorStruct";
        case SkSL::Expression::Kind::kEmpty:                     return "Empty";
        case SkSL::Expression::Kind::kFieldAccess:               return "FieldAccess";
        case SkSL::Expression::Kind::kFunctionReference:         return "FunctionReference";
        case SkSL::Expression::Kind::kFunctionCall:              return "FunctionCall";
        case SkSL::Expression::Kind::kIndex:                     return "Index";
        case SkSL::Expression::Kind::kLiteral:                   return "Literal";
        case SkSL::Expression::Kind::kMethodReference:           return "MethodReference";
        case SkSL::Expression::Kind::kPoison:                    return "Poison";
        case SkSL::Expression::Kind::kPostfix:                   return "Postfix";
        case SkSL::Expression::Kind::kPrefix:                    return "Prefix";
        case SkSL::Expression::Kind::kSetting:                   return "Setting";
        case SkSL::Expression::Kind::kSwizzle:                   return "Swizzle";
        case SkSL::Expression::Kind::kTernary:                   return "Ternary";
        case SkSL::Expression::Kind::kTypeReference:             return "TypeReference";
        case SkSL::Expression::Kind::kVariableReference:         return "VariableReference";
    }
    SkUNREACHABLE;
}
...
static std::string summarize_description(std::string_view description, size_t maxChars = 120) {
    std::string summary = collapse_whitespace(description);
    if (summary.size() <= maxChars) {
        return summary;
    }
    if (maxChars <= 3) {
        return summary.substr(0, maxChars);
    }
    summary.resize(maxChars - 3);
    summary += "...";
    return summary;
}

static std::string position_summary(const SkSL::Position& position, const std::string* source) {
    if (!position.valid()) {
        return "";
    }

    std::string result = "offset=" + std::to_string(position.startOffset());
    result += ".." + std::to_string(position.endOffset());
    if (source) {
        result += " line=" + std::to_string(position.line(*source));
    }
    return result;
}

static std::string owned_program_description(const SkSL::Program& program) {
    std::string result = program.fConfig->versionDescription();
...
class IRTreeDumper final : public SkSL::ProgramVisitor {
public:
    explicit IRTreeDumper(SkSL::OutputStream& out) : fOut(out) {}

    void dump(const SkSL::Program& program) {
        fSource = program.fSource.get();
        this->writeLine(0,
                        "Program kind=" + std::string(program_kind_name(program.fConfig->fKind)) +
                                " elements=" + std::to_string(program.fOwnedElements.size()) +
                                " builtin_elements=omitted");

        fDepth = 1;
        for (const std::unique_ptr<SkSL::ProgramElement>& element : program.fOwnedElements) {
            this->visitProgramElement(*element);
        }
    }

protected:
    bool visitProgramElement(const SkSL::ProgramElement& element) override {
        this->writeLine(
                fDepth,
                "ProgramElement " + std::string(program_element_kind_name(element.kind())) +
                        this->format_position(element.position()) +
                        this->format_summary(element.description()));
        ++fDepth;
        bool result = INHERITED::visitProgramElement(element);
        --fDepth;
        return result;
    }

    bool visitStatement(const SkSL::Statement& statement) override {
        this->writeLine(
                fDepth,
                "Statement " + std::string(statement_kind_name(statement.kind())) +
                        this->format_position(statement.position()) +
                        this->format_summary(statement.description()));
        ++fDepth;
        bool result = INHERITED::visitStatement(statement);
        --fDepth;
        return result;
    }

    bool visitExpression(const SkSL::Expression& expression) override {
        this->writeLine(fDepth,
                        "Expression " + std::string(expression_kind_name(expression.kind())) +
                                " type=" + expression.type().description() +
                                this->format_position(expression.position()) +
                                this->format_summary(expression.description()));
        ++fDepth;
        bool result = INHERITED::visitExpression(expression);
        --fDepth;
        return result;
    }

private:
    std::string format_position(const SkSL::Position& position) const {
        std::string summary = position_summary(position, fSource);
        return summary.empty() ? "" : " [" + summary + "]";
    }

    std::string format_summary(std::string_view description) const {
        std::string summary = summarize_description(description);
        return summary.empty() ? "" : " : " + summary;
    }

    void writeLine(int depth, const std::string& text) {
        for (int index = 0; index < depth; ++index) {
            fOut.writeText("  ");
        }
        fOut.writeText(text.c_str());
        fOut.writeText("\n");
    }

    SkSL::OutputStream& fOut;
    const std::string* fSource = nullptr;
    int fDepth = 0;

    using INHERITED = SkSL::ProgramVisitor;
};
...
static void show_usage() {
    printf("usage: skslc <input> <output> <flags>\n"
           ...
           ".ast, .glsl, .hlsl, .ir, .metal, .spirv, .asm.vert, .asm.frag,\n"
           ".asm.comp, .wgsl, .skrp, .stage\n");
}
...
static ResultCode process_command(SkSpan<std::string> args) {
    ...
    if (skstd::ends_with(outputPath, ".ast")) {
        return compileProgram([](SkSL::Compiler&,
                                 const SkSL::ShaderCaps*,
                                 SkSL::Program& program,
                                 SkSL::OutputStream& out) {
            IRTreeDumper dumper(out);
            dumper.dump(program);
            return true;
        });
    } else if (skstd::ends_with(outputPath, ".ir")) {
        return compileProgram([](SkSL::Compiler&,
                                 const SkSL::ShaderCaps*,
                                 SkSL::Program& program,
                                 SkSL::OutputStream& out) {
            out.writeString(SkShaderUtils::PrettyPrint(owned_program_description(program)));
            return true;
        });
    } else if (skstd::ends_with(outputPath, ".spirv")) {
        ...
        return compileProgram([&, inputPath](SkSL::Compiler& compiler,
                                 const SkSL::ShaderCaps* shaderCaps,
                                 SkSL::Program& program,
                                 SkSL::OutputStream& out) {
            // Try to load an input provenance config from a prior GLSL→SkSL translation.
            // This carries metadata (shader stage, dialect, version, etc.) that guides
            // the SkSL→GLSL output for round-trip fidelity.
            sksl_glsl_binding::ProvenanceConfig inConfig;
            std::string provInputPath = inputPath + ".provenance";
            std::ifstream provIn(provInputPath);
            if (provIn.good()) {
                std::string json((std::istreambuf_iterator<char>(provIn)),
                                 std::istreambuf_iterator<char>());
                if (!json.empty()) {
                    inConfig.fromJson(json);
                }
            }

            bool ok = SkSL::ToGLSL(program, shaderCaps, out, SkSL::PrettyPrint::kYes,
                                   inConfig.empty() ? nullptr : &inConfig);
            // Write provenance sideband file for round-trip fidelity
            const auto& prov = SkSL::GetLastProvenanceConfig();
            if (ok && !prov.empty()) {
                std::string provPath = outputPath + ".provenance";
        ...
        printf("expected output path to end with one of: .ast, .glsl, .hlsl, .ir, .metal, "
               ".wgsl, .spirv, .asm.vert, .asm.frag, .asm.comp, .skrp, .stage (got '%s')\n",
               outputPath.c_str());
