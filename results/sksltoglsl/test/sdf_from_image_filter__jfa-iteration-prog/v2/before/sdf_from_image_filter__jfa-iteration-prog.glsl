...
vec4 SafeFetch_f4f2(vec2 fragCoord) {
...
void SearchNeighbors_vf4f2f2(inout vec4 O, vec2 fragCoord, vec2 sampleCoord) {
    vec4 imgSample = SafeFetch_f4f2(sampleCoord);
...
void main() {
    ...
    SearchNeighbors_vf4f2f2(O, fragCoord, fragCoord + jfaRadius * vec2(0.0, -1.0));
    SearchNeighbors_vf4f2f2(O, fragCoord, fragCoord + jfaRadius * vec2(0.0, 1.0));
    SearchNeighbors_vf4f2f2(O, fragCoord, fragCoord + jfaRadius * vec2(-1.0, 0.0));
    SearchNeighbors_vf4f2f2(O, fragCoord, fragCoord + jfaRadius * vec2(1.0, 0.0));
