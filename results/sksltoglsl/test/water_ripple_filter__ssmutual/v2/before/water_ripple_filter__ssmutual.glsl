...
vec2 waveGenerator_f2ffffff(float propDis, float t, float count, float freq, float prop, float yShift) {
...
void main() {
    ...
    float b_ampDecayByDis = b_propDis < 1.9 ? clamp(pow(1.9 - b_propDis, b_decayExp), 0.0, 1.0) : 0.0;
    float s_ampDecayByDis = s_propDis < 0.7 ? clamp(pow(0.7 - s_propDis, s_decayExp), 0.0, 1.0) : 0.0;
    float s_ampSupCenter = smoothstep(0.0, s_ampSupArea, s_propDis);
    vec2 b_waveRes = waveGenerator_f2ffffff(b_propDis, b_t, 1.0, b_waveFreq, b_wavePropRatio, 1.0);
    vec2 s_waveRes = waveGenerator_f2ffffff(s_propDis, s_t, s_waveCount, s_waveFreq, s_wavePropRatio, abs(normalize(s_vec).y));
    float b_intense = (b_waveRes.x * b_ampDecayByDis) * b_ampSupress;
    float s_intense = ((s_waveRes.x * s_ampDecayByDis) * s_ampSupCenter) * s_ampSupress;
    float b_Prime = (b_waveRes.y * b_ampDecayByDis) * b_ampSupress;
    float s_Prime = ((s_waveRes.y * s_ampDecayByDis) * s_ampSupCenter) * s_ampSupress;
    ...
    vec2 warp = (b_intensity * b_norm.xy + s_intensity * s_norm.xy) * smoothstep(0.0, 0.07, progress);
    vec2 expandUV = (uv - warp) * iResolution;
    vec3 color = texture(image, (expandUV) / iResolution).xyz;
    float b_light = (b_luminance * clamp(b_Prime, 0.0, 1.0)) * smoothstep(0.0, 0.125, progress);
    float s_light = s_luminance * clamp(s_Prime, 0.0, 1.0);
    color += s_light;
    const vec3 _0_oneVec = vec3(1.0);
    color = _0_oneVec - (_0_oneVec - color) * (_0_oneVec - vec3(b_light));
